        ...
        public void onReceive(byte[] p_data, String p_ip, int p_port) {
            ...
            if (isCode)
                switch (RequestCode.fromPacket(p_data)) {
                    // TODO: Make clients send their names with this request!
                    ...
                    case ADD_ME: { // Limits the stack so that `client` is namespaced here :D
                        ...
                        try {
                            ...
                            names = PApplet.split(new String(p_data, Integer.BYTES, strEnd), // p_data.length /
                                                                                             // Character.BYTES),
                                    '\n');
                            client = socket.new AgcClient(socket, p_ip, p_port, names[0], names[1]);
                        } catch (Exception e) {
                            String name = new String(p_data, Integer.BYTES, p_data.length / Character.BYTES);
                            ...
                            System.out.printf("The client of IP `%s`, reported only one name, \"%s\".", p_ip, name);
                        }
